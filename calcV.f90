subroutine calc_v(triangles, sigmas, E0, x, y, z, N, num, div, V)  ! triangles:三角形のデータ, num:三角形数, sigmas:電荷密度, x,y,zは求めたい電位の座標, Vは返す電位
    implicit none
    integer i, j, k, N, num, div
    double precision triangles(num,3,3), eps0, pi, E0(3), r(3), sigmas(num), &
                        & x(div), y(div), z(div), V(div,div,div)
    intent(in) triangles, num, div, sigmas, x, y, z, N, E0
    intent(out) V
    ...
    do i = 1,100
        ...
        do j = 1,100
            do k = 1,100
                r = [x(i), y(j), z(k)]
                V = potential(r, triangles, sigmas)
            end do
        end do
    end do
