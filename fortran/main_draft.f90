...
100 continue
    ...
    E0(1,:) = [0, 0, 0]     ! background electric field

    ! calculate coeffient matrix
    do i = 1, num_of_triangles
        do j = 1, num_of_triangles
            if (i /= j) then
                P(j,i) = p_calc(triangles(i), incenter(triangles(j)%coord))
            else
                P(j,i) = p_calc_o(triangles(i)%coord)
            end if
        end do
    end do

    ! take into account background electric field
    do i = 1, num_of_triangles
        
    end do

    ! solve linear equatins (¿ LU decomposition ?)
