program main
    implicit none
    integer i, j, num_of_rectangles, num_of_triangles, info, num_of_faces
        ...
        if (i<=num_of_rectangles) then
            do j = 1, num_of_faces
                if (j<=num_of_rectangles) then
                    ...
                    P(j,i) = p_calc_rect(rectangles(i), centroid_triangle(triangles(j-num_of_rectangles)))
                end if
            end do
        else
            do j = 1, num_of_faces
                if (j<=num_of_rectangles) then
                    P(j,i) = p_calc_tri(triangles(i-num_of_rectangles), centroid_rectangle(rectangles(j)))
                else
                    P(j,i) = p_calc_tri(triangles(i-num_of_rectangles), centroid_triangle(triangles(j-num_of_rectangles)))
                end if
            end do
        end if
    end do
    ...
    ! solve linear system
    call dgesv(num_of_faces, 1, P, num_of_faces, ipiv, sigmas, num_of_faces, info)
    ...
    do i = 1, num_of_faces
        if (i<=num_of_rectangles) then
            ...
            Q = Q + sigmas(i)*area_tri(triangles(i-num_of_rectangles))
        end if
    end do
